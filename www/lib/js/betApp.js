...
function ajax($scope, $http) {
    ...
    .success(function (data, status, jq) {
        if (jq.status === 200) { //authentification réussie
            ...
            alert(jq.status + "Nom d'utilisateur ou mot de passe incorrect. Essayez à nouveau.");
            console.log(JSON.stringify(data));
        }
    })
    .error(function (jq, status, error) {
        alert("Erreur lors de l'appel du WebService. Consulter la console pour toute information complémentaire.");
        console.log(JSON.stringify(status + " " + error));
    });
